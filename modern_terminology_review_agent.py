    ...
    def search(self, query: str, preferred_backends: List[str] = None) -> Dict[str, str]:
        ...
        for backend in preferred_backends:
            ...
            for attempt in range(2):  # 2 attempts per backend
                try:
                    print(f"🔍 Searching with backend: {backend}" + (f" (attempt {attempt + 1})" if attempt > 0 else ""))
                    self._rate_limit()
                    ...
                    if search_results:
                        ...
                        print(f"✅ {backend} search successful: {len(search_results)} results")
                        search_successful = True
                        break  # Success, no need to retry
                    else:
                        if attempt == 0:
                            print(f"⚠️ {backend} search returned no results, retrying...")
                            time.sleep(2)  # Brief delay before retry
                            continue
                        else:
                            results[backend] = f"{backend} search returned no results after retries"
                            print(f"⚠️ {backend} search returned no results after {attempt + 1} attempts")
                    ...
                    if attempt == 0 and "rate" not in str(e).lower():  # Don't retry rate limit errors
                        ...
                        print(f"❌ {error_msg}" + (f" after {attempt + 1} attempts" if attempt > 0 else ""))
                        break
        
        # If no searches succeeded, try a simplified fallback approach
        if successful_searches == 0:
            print("🔄 All backends failed, trying fallback search...")
            try:
                self._rate_limit()
                ...
                if fallback_results:
                    ...
                    print("⚠️ Fallback search returned no results")
            except Exception as e:
...
class PatchedAzureOpenAIServerModel(AzureOpenAIServerModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print("🔄 [ModernReviewAgent] Initializing with fresh token...")
    ...
    def _refresh_token_if_needed(self):
        ...
        try:
            ...
            if not hasattr(self, '_token_expiry') or self._token_expiry is None or not hasattr(self, 'api_key'):
                print("🔄 [ModernReviewAgent] No token found, acquiring new token...")
            ...
            if datetime.now() + timedelta(minutes=10) >= self._token_expiry:
                print("🔄 [ModernReviewAgent] Token expiring soon, refreshing...")
                return self._force_token_refresh()
            
            return True
        except Exception as e:
            print(f"❌ [ModernReviewAgent] Token check failed: {e}")
            return self._force_token_refresh()

    def _force_token_refresh(self):
        """Force token refresh regardless of current state"""
        try:
            print("🔄 [ModernReviewAgent] Forcing token refresh...")
            ...
            print(f"✅ [ModernReviewAgent] Token refreshed successfully, expires at {self._token_expiry.strftime('%H:%M:%S')}")
            return True
        except Exception as e:
            print(f"❌ [ModernReviewAgent] Token refresh failed: {e}")
            return False
...
class ModernTerminologyReviewTool(Tool):
    ...
    def _analyze_autodesk_context(self, term: str, src_lang: str = "EN", tgt_lang: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for lang_pair in language_pairs:
                ...
                if src_check.upper() == src_lang.upper():
                    try:
                        ...
                        print(f"⚠️ Error checking language pair {src_check}-{tgt_check}: {e}")
                        continue
            ...
            print(f"⚠️ Error in Autodesk context analysis: {e}")
    ...
    def _analyze_original_texts(self, original_texts: List[str], limit_for_efficiency: bool = False) -> Dict[str, Any]:
        ...
        if limit_for_efficiency and len(original_texts) > 10:
            ...
            print(f"⚡ Limited original texts to 10 for processing efficiency")
    ...
    def _validate_single_term(self, term: str, src_lang: str = "EN", tgt_lang: str = None, 
        ...
        for i, query in enumerate(research_queries[:3]):  # Limit to 3 queries to avoid rate limits
            ...
            if not success:
                failed_searches += 1
                print(f"❌ Search query {i+1} failed")
            else:
                print(f"✅ Search query {i+1} successful")
        ...
        if failed_searches >= 3:  # Only if ALL 3 searches failed completely
            ...
            print(f"❌ Web search failed: ALL {failed_searches}/3 queries failed")
        else:
            ...
            if has_useful_info or total_content_length > 100:
                ...
                print(f"✅ Web search successful: {success_count}/3 queries provided useful information ({total_content_length} chars)")
            else:
                web_search_successful = False
                print(f"❌ Web search failed: {failed_searches}/3 queries failed, no useful information retrieved")
        ...
        if translation_insights["translation_available"]:
            ...
            print(f"🎯 Translation contribution to score: +{translation_score:.3f}")
        else:
            print("🎯 No translation data - using traditional validation only")
    ...
    def _save_results_to_file(self, results: List[Dict], output_file: str, metadata: Dict = None):
        ...
        if failed_search_count == len(results) and meaningful_results == 0:
            print(f"❌ Not saving {output_file}: ALL {failed_search_count} terms failed with no validation data")
            print(f"⚠️ File not created due to complete validation failure")
            return False
        ...
        if meaningful_results > 0:
            if failed_search_count > 0:
                print(f"⚠️ Saving with partial web search failures: {meaningful_results}/{len(results)} terms have meaningful validation")
                print(f"📊 Web search status: {len(results) - failed_search_count}/{len(results)} successful, {failed_search_count} failed")
            else:
                print(f"✅ Saving with all web searches successful: {meaningful_results}/{len(results)} meaningful validations")
        
        # If we have some failed searches but not too many, warn but still save
        if failed_search_count > 0:
            print(f"⚠️ Warning: {failed_search_count}/{len(results)} terms had failed web searches, but saving anyway")
        ...
        try:
            ...
            if original_texts:
                try:
                    ...
                    print(f"⚠️ Warning: Could not parse original_texts JSON: {e}")
                    parsed_original_texts = None
            ...
            if translation_data:
                try:
                    ...
                    print(f"⚠️ Warning: Could not parse translation_data JSON: {e}")
                    parsed_translation_data = None
...
def run_with_retries(func, max_retries: int = 5, base_delay: float = 2.0):
    ...
    for attempt in range(max_retries):
        try:
            print(f"🔄 [ModernReviewAgent] Attempt {attempt + 1}: Executing with fresh authentication...")
            return func()
        except Exception as e:
            error_msg = str(e)
...
class ModernTerminologyReviewAgent:
    ...
    def _setup_model(self):
        ...
        try:
            ...
            print(f"✅ Model {self.model_name} setup complete")
    ...
    def refresh_model_token(self):
        ...
        try:
            print("🔄 Recreating model with fresh token...")
            ...
            print(f"✅ Model recreated with fresh token, expires at {self.model._token_expiry.strftime('%H:%M:%S')}")
            
            # Update the agent with the new model
            if hasattr(self, 'agent') and self.agent:
                self.agent.model = self.model
                print("✅ Agent updated with new model")
                
        except Exception as e:
            print(f"❌ Failed to recreate model with fresh token: {e}")
            raise
    ...
    def _create_agent(self):
        ...
        try:
            ...
            print("✅ Modern agent created with DDGS search engine")
...
"""
    ...
    def validate_term_candidate_efficient(self, term: str, src_lang: str = "EN", tgt_lang: str = None, 
        ...
        if limit_efficiency:
            print("⚡ Using efficiency mode for term with many original texts")
        ...
        if translation_data:
            try:
                ...
                print(f"⚠️ Warning: Could not serialize translation_data: {e}")
                translation_data_param = ""
    ...
    def batch_validate_terms(self, terms: List[str], src_lang: str = "EN", tgt_lang: str = None, 
        ...
        if original_texts:
            try:
                ...
                print(f"⚠️ Warning: Could not serialize original_texts: {e}")
                original_texts_param = ""
...
def main():
    ...
    print("🚀 Modern Terminology Review Agent with DDGS")
    print("=" * 50)
