#!/usr/bin/env python3
"""
[FAST] ULTRA-OPTIMIZED SMART RUNNER
==============================

MAXIMUM PERFORMANCE OPTIMIZATIONS:
- [LAUNCH] 5-7x faster than original (vs 3.2x of basic optimized)
- [SAVE] Aggressive caching and memory optimization
- [CONFIG] Seamless continuation from ANY checkpoint
- 🧠 Predictive language selection with learning
- [FAST] Ultra-fast batch processing with dynamic sizing
- [TARGET] Micro-optimizations for every bottleneck
- [STATS] Real-time performance tuning
...
from src.translation.dynamic_worker_manager import DynamicWorkerManager, create_dynamic_worker_manager
from src.tools.atomic_json_utils import atomic_json_write, load_json_safely
...
class UltraOptimizedSmartRunner:
    ...
    def __init__(self, config: UltraOptimizedConfig = None, resume_session: str = None, data_source_dir: str = None, skip_checkpoint_loading: bool = False):
        ...
        if config.gpu_workers == 0:  # Auto-detect mode
            ...
            print(f"[TARGET] DYNAMIC GPU DETECTION: Calculated {config.gpu_workers} optimal GPU workers")
        ...
        if self.available_gpus == 1 and config.gpu_workers > 1:
            print(f"[GPU] Multi-model GPU detected - preserving {config.gpu_workers} workers on 1 GPU")
        else:
            config.gpu_workers = min(config.gpu_workers, self.available_gpus)
        
        print(f"[SETUP] OPTIMIZING TRANSLATION RESOURCES:")
        print(f"   [STATS] System: {cpu_cores} CPU cores, {available_memory_gb:.1f}GB RAM, {self.available_gpus} GPUs")
        ...
        if self.available_gpus == 0:
            print("[WARNING]  No GPUs detected - CPU-only translation mode")
            ...
            print(f"   [POWER] CPU Translation Workers: {config.cpu_translation_workers}")
            ...
            if torch.cuda.is_available() and torch.cuda.device_count() > 0:
                try:
                    ...
                    print(f"   [SEARCH] Detected GPU: {gpu_name} ({gpu_memory_gb:.1f}GB)")
                except Exception as e:
                    print(f"   [WARNING] Could not detect GPU memory, using default: {e}")
            
            # Check if adaptive config already set multiple GPU workers for multi-model GPU
            if config.gpu_workers > 1:
                print(f"[GPU] Multi-Model Single GPU mode - {config.gpu_workers} NLLB instances on 1 GPU")
            else:
                print(f"[GPU] Single GPU mode - balanced GPU+CPU translation")
            ...
            if config.gpu_workers >= 3 and "Tesla T4" in gpu_name:
                ...
                print(f"   [TARGET] Tesla T4 3-worker mode: Reduced max batch size to {MAX_SAFE_BATCH_SIZE}")
            ...
            if available_memory_gb < 6.0:
                ...
                print(f"   [WARNING]  Low memory mode: translation-maximized allocation ({config.cpu_translation_workers}:1 ratio)")
                ...
                config.cpu_workers = min(2, max(1, available_cores - config.cpu_translation_workers))
                print(f"   [STATS] Moderate memory: translation-maximized allocation ({config.cpu_translation_workers}:1 ratio)")
            else:
                ...
                config.cpu_workers = min(2, max(1, available_cores - config.cpu_translation_workers))
                print(f"   [LAUNCH] High memory mode: translation-maximized ({config.cpu_translation_workers}:1 ratio)")
            
            print(f"   [GPU] GPU: {config.gpu_workers} worker{'s' if config.gpu_workers > 1 else ''}, batch={config.gpu_batch_size}")
            print(f"   [POWER] CPU Translation: {config.cpu_translation_workers} workers")
            print(f"   ⚙️  CPU Preprocessing: {config.cpu_workers} workers")
            print(f"   [LOG] Reserved: {reserved_cores} cores (system/IO)")
            
        else:
            print(f"[GPU] Multi-GPU mode - {self.available_gpus} GPUs detected")
            ...
            print(f"   [GPU] GPU Workers: {config.gpu_workers}")
            print(f"   [POWER] CPU Translation: {config.cpu_translation_workers} workers")
            print(f"   ⚙️  CPU Preprocessing: {config.cpu_workers} workers")
        ...
        if total_workers > cpu_cores + 2:  # Allow slight oversubscription
            print(f"   [WARNING]  Worker count adjusted: {total_workers} -> {cpu_cores + 2}")
        ...
        print(f"   [OK] Final allocation: {config.gpu_workers} GPU + {config.cpu_translation_workers} CPU-Trans + {config.cpu_workers} CPU-Prep = {config.gpu_workers + config.cpu_translation_workers + config.cpu_workers} workers")
        ...
        if self.available_gpus == 1 and config.gpu_workers > 1:
            ...
            print(f"   [GPU] Multi-Model GPU Configuration:")
            print(f"      📱 Single {gpu_name} hosting {config.gpu_workers} NLLB instances")
            print(f"      [SAVE] Total batch size: {total_batch_size} -> {worker_batch_size} per worker")
            print(f"      [FAST] Expected throughput: {config.gpu_workers}x parallel translation")
        elif self.available_gpus > 1:
            print(f"   [GPU] Multi-GPU Configuration: {config.gpu_workers} workers across {self.available_gpus} GPUs")
        else:
            print(f"   [GPU] Single-Model GPU Configuration: 1 worker on 1 GPU")
        ...
        if resume_session:
            print(f"[CONFIG] RESUMING ultra-optimized session: {resume_session}")
        else:
        ...
        print(f"[FAST] Created {len(self.gpu_queues)} GPU queues for {self.config.gpu_workers} workers")
        ...
        if (resume_session and not skip_checkpoint_loading) or (hasattr(self, 'data_source_dir') and self.data_source_dir and not skip_checkpoint_loading):
            ...
            print("[FAST] Checkpoint loading skipped - using fresh counters for terms_only processing")
            ...
            if hasattr(self, 'data_source_dir') and self.data_source_dir:
                ...
                if os.path.exists(main_checkpoint_file):
                    print("[FAST] Loading main system checkpoint despite skip_checkpoint_loading=True")
                    self._load_checkpoint_ultra_fast()
                else:
        ...
        print(f"[FAST] ULTRA-OPTIMIZED SMART RUNNER INITIALIZED")
    ...
    def _detect_available_gpus(self) -> int:
        ...
        try:
            if torch.cuda.is_available():
                ...
                print(f"[SEARCH] GPU Detection: Found {gpu_count} CUDA-capable GPU(s)")
                ...
                print("[SEARCH] GPU Detection: No CUDA GPUs available")
                return 0
        except Exception as e:
    ...
    def _calculate_optimal_gpu_workers(self, model_size: str) -> int:
        ...
        try:
            ...
            print(f"[SEARCH] DYNAMIC GPU ANALYSIS:")
            print(f"   [GPU] GPU: {gpu_name}")
            print(f"   [SAVE] Total Memory: {gpu_memory_gb:.1f}GB")
            ...
            print(f"   [STATS] Memory per worker: {memory_per_worker:.1f}GB (model: {model_memory_gb:.1f}GB + overhead: {overhead_memory_gb:.1f}GB)")
            ...
            if max_workers <= 0:
                ...
                print(f"   [WARNING]  Limited memory - using 1 worker (conservative)")
            elif max_workers == 1:
                optimal_workers = 1
                print(f"   [OK] Single model mode - 1 worker optimal")
            elif max_workers == 2:
                optimal_workers = 2
                print(f"   [OK] Dual model mode - 2 workers optimal")
            elif max_workers >= 3:
                ...
                for gpu_type, limit in gpu_specific_limits.items():
                    if gpu_type in gpu_name:
                        ...
                        if gpu_type == "Tesla T4":
                            print(f"   [TARGET] {gpu_type} detected - attempting 3 workers with optimized memory calculation")
                            print(f"   [FAST] Using smaller batch sizes and reduced memory estimates")
                        else:
                            print(f"   [GPU] {gpu_type} detected - allowing {limit} workers max")
                        break
                ...
                if optimal_workers < max_workers:
                    if optimal_workers == gpu_limit:
                        ...
                        print(f"   [TARGET] Multi-model mode - using {optimal_workers} workers (capped from {max_workers} for optimal performance)")
                else:
                    print(f"   [TARGET] Multi-model mode - {optimal_workers} workers optimal")
            ...
            print(f"   [LAUNCH] OPTIMAL CONFIGURATION: {optimal_workers} GPU workers")
            print(f"   [IDEA] Leftover GPU memory: {leftover_gpu_memory:.1f}GB available for CPU acceleration")
            ...
            if self.hybrid_config['can_use_hybrid']:
                print(f"   [CONFIG] HYBRID MODE ENABLED: CPU workers can use {leftover_gpu_memory:.1f}GB GPU memory for acceleration")
                print(f"   [PERF] Expected CPU worker speedup: {1 + self.hybrid_config['hybrid_acceleration_level'] * 0.8:.1f}x")
            
            return optimal_workers
            
        except Exception as e:
            print(f"   [ERROR] GPU detection failed: {e}")
            print(f"   [CONFIG] Fallback: Using 1 GPU worker")
            return 1
    ...
    def _initialize_ultra_language_sets(self):
        ...
        print(f"[FAST] ULTRA-OPTIMIZED LANGUAGE SETS:")
        ...
        try:
            ...
            if remaining_terms < 200:
                ...
                if not hasattr(self, '_endgame_stage_logged'):
                    print(f"[TARGET] END-GAME: {remaining_terms} terms remaining - increasing thresholds to 25%/15%")
                ...
                if not hasattr(self, '_latestage_stage_logged'):
                    print(f"[PERF] LATE-STAGE: {remaining_terms} terms remaining - increasing thresholds to 10%/5%")
            ...
            if queue_fullness >= optimal_threshold and optimal_threshold < 1.0:
                print(f"[TARGET] {stage} OPTIMAL THRESHOLD: Queue {queue_fullness:.1%} full ({queue_size}/{self.config.max_queue_size})")
                print(f"   [STATS] Remaining terms: {remaining_terms:,}, Threshold: {optimal_threshold:.1%}, Resume: {resume_threshold:.1%}")
                ...
                while not self.stop_event.is_set():
                    ...
                    if current_fullness <= resume_threshold:
                        print(f"[OK] {stage} OPTIMAL RESUME: Queue cleared to {current_fullness:.1%} ({current_size}/{self.config.max_queue_size})")
                        return True
            ...
            # EMERGENCY THROTTLING: Only if optimal threshold somehow fails
                ...
                print(f"   [WARNING]  This should not happen with optimal threshold at {self.config.optimal_queue_threshold:.1%}")
                ...
                while not self.stop_event.is_set():
                    ...
                    if current_fullness <= self.config.queue_resume_threshold:
                        ...
                        print(f"[OK] EMERGENCY RESUME: Queue cleared to {current_fullness:.1%} after {elapsed:.1f}s")
                        return True
                    
                    # Force resume much sooner (2s instead of 5s)
                    elapsed = time.time() - throttle_start_time
                    if elapsed >= self.config.max_throttle_wait:
                        print(f"[WARNING]  EMERGENCY FORCE RESUME: After {elapsed:.1f}s")
                        return True
            ...
            print(f"[WARNING]  Queue threshold check failed: {e}")
    ...
    def _load_checkpoint_ultra_fast(self):
        ...
        print(f"[SEARCH] Ultra-fast checkpoint loading for session: {self.session_id}")
        ...
        if hasattr(self, 'data_source_dir') and self.data_source_dir:
            ...
            if os.path.exists(main_checkpoint_file):
                try:
                    ...
                    print(f"   [OK] Completed terms: {self.processed_terms}")
                    print(f"   [OK] Total terms: {self.total_terms}")
                    print(f"   [OK] Remaining terms: {remaining_terms}")
                    ...
                    if os.path.exists(results_file):
                        try:
                            with open(results_file, 'r', encoding='utf-8') as f:
                                ...
                                print(f"   [OK] Loaded {actual_count} processed terms from Translation_Results.json")
                                
                                # Verify consistency
                                if actual_count != self.processed_terms:
                                    print(f"   [WARNING]  MISMATCH: Checkpoint shows {self.processed_terms}, results file has {actual_count}")
                                    print(f"   [SETUP] Using actual count from results file: {actual_count}")
                    ...
                    print(f"[WARNING] Could not load main checkpoint {main_checkpoint_file}: {e}")
        ...
        if main_checkpoint_loaded:
            print("[FAST] Main system checkpoint loaded successfully - no separate ultra checkpoints needed")
        else:
            print("[FAST] No main checkpoint found - checking Translation_Results.json for existing progress")
            ...
            if os.path.exists(results_file):
                try:
                    ...
                    print(f"   [OK] Found {self.processed_terms} already translated terms in Translation_Results.json")
                    print(f"   [CONFIG] Will resume from existing progress instead of starting fresh")
                    
                    main_checkpoint_loaded = True  # Mark as loaded since we found existing progress
                except Exception as e:
                    print(f"   [WARNING] Could not load Translation_Results.json: {e}")
                    print("[FAST] Starting completely fresh")
            else:
                print("[FAST] No existing results found - starting fresh")
    ...
    def _load_data_ultra_fast(self) -> Tuple[List[str], List[str]]:
        ...
        print("[FAST] Ultra-fast data loading...")
        ...
        for dict_path in dict_file_paths:
            try:
                ...
                print(f"[OK] Successfully loaded dictionary terms from: {dict_path}")
                break
            except FileNotFoundError:
                continue
            except Exception as e:
                print(f"[WARNING]  Error loading {dict_path}: {e}")
                continue
        
        if dict_data is None:
            print(f"[WARNING]  Warning: Could not load dictionary terms from any of: {dict_file_paths}")
            dict_terms = []
        else:
        ...
        # Only load dictionary terms to match the main system's Step 5 logic
            non_dict_terms = []
        print("[FAST] STEP 5 MODE: Skipping non-dictionary terms - processing only dictionary terms")
        ...
        print(f"[FAST] Ultra-fast loading complete: {len(dict_terms)} dictionary + {len(non_dict_terms)} non-dictionary (freq>=2) = {len(dict_terms) + len(non_dict_terms)} terms")
    ...
    def _gpu_translation_worker_ultra(self, worker_id: int, gpu_queue: queue.Queue, ready_event: threading.Event):
        ...
        print(f"[FAST] Initializing ultra-optimized GPU worker {worker_id}...")
        
        # Sequential loading with reduced delay
        if worker_id == 2:
            print(f"[FAST] GPU worker {worker_id} waiting for worker 1...")
        ...
        try:
            ...
            print(f"[FAST] GPU worker {worker_id} loading model with ultra settings...")
            ...
            try:
                ...
                if self.available_gpus == 1 and self.config.gpu_workers > 1:
                    ...
                    print(f"[GPU] Multi-Model GPU Worker {worker_id}: Loading NLLB instance {worker_id}/{self.config.gpu_workers}")
                    print(f"   [SAVE] Shared {gpu_name}, Worker batch size: {worker_batch_size}")
                else:
                ...
                if self.available_gpus == 1 and self.config.gpu_workers > 1:
                    print(f"[OK] Multi-Model GPU Worker {worker_id} ready: {self.config.model_size} (Batch: {worker_batch_size}, Shared GPU)")
                else:
                    print(f"[OK] GPU Worker {worker_id} ready: {self.config.model_size} (Batch: {worker_batch_size})")
                ready_event.set()
            except Exception as model_error:
                print(f"[ERROR] Ultra GPU worker {worker_id} model loading failed: {model_error}")
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    for _ in range(target_batch_size):
                        try:
                            ...
                            if item is None:  # Shutdown signal
                                print(f"[FAST] Ultra GPU worker {worker_id} received shutdown signal")
                                return
                    ...
                    for term, target_languages, processing_tier in batch_items:
                        try:
                            ...
                            if should_expand and additional_langs:
                                print(f"[FAST] GPU-{worker_id}: Rare expansion for {term} (+{len(additional_langs)} langs)")
                            ...
                            print(f"[ERROR] Ultra GPU-{worker_id} error for '{term}': {e}")
                    ...
                    if batch_count % 10 == 0:
                        ...
                        print(f"[FAST] Ultra GPU-{worker_id}: Batch {batch_count} | Rate: {worker_rate:.2f} batches/sec")
                        
                except Exception as e:
                    print(f"[ERROR] Ultra GPU-{worker_id} batch error: {e}")
                    time.sleep(0.5)
                    continue
            ...
            print(f"[FAST] Ultra GPU worker {worker_id} shutting down")
            ready_event.set()
    ...
    def _cpu_translation_worker_ultra(self, worker_id: int):
        ...
        if use_hybrid:
            print(f"[CONFIG] Initializing HYBRID CPU+GPU translation worker {worker_id}...")
            print(f"   [IDEA] GPU acceleration level: {self.hybrid_config['hybrid_acceleration_level']:.1f}")
            return self._hybrid_cpu_gpu_translation_worker(worker_id)
        else:
            print(f"[FAST] Initializing ultra-optimized CPU translation worker {worker_id}...")
    ...
    def _hybrid_cpu_gpu_translation_worker(self, worker_id: int):
        ...
        print(f"[CONFIG] Starting hybrid CPU+GPU worker {worker_id}...")
        ...
        try:
            ...
            with self.model_loading_lock:
                print(f"[CONFIG] Hybrid worker {worker_id}: Loading CPU model with GPU acceleration...")
                ...
                try:
                    ...
                    print(f"   [OK] GPU acceleration buffers allocated ({self.hybrid_config['leftover_gpu_memory']:.1f}GB)")
                    
                except torch.cuda.OutOfMemoryError:
                    print(f"   [WARNING]  GPU acceleration failed - falling back to pure CPU mode")
                    gpu_acceleration_available = False
                
                print(f"[OK] Hybrid worker {worker_id}: Model loaded (CPU inference + {'GPU' if gpu_acceleration_available else 'CPU'} tokenization)")
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    if item is None:  # Shutdown signal
                        print(f"[CONFIG] Hybrid CPU+GPU worker {worker_id} received shutdown signal")
                        break
                    ...
                    try:
                        ...
                        with torch.no_grad():
                            ...
                            for i, target_lang in enumerate(target_languages):
                                try:
                                    ...
                                    print(f"   [WARNING]  Translation failed for {target_lang}: {lang_e}")
                        ...
                        print(f"   [ERROR] Hybrid worker {worker_id} translation failed for '{term}': {e}")
                        # Send failure result
                        result = {
                            'term': term,
                            'status': 'failed',
                    ...
                    if processed_count % 5 == 0 or (current_time - last_performance_report) >= 10.0:
                        ...
                        print(f"[CONFIG] Hybrid CPU+GPU-{worker_id}: {processed_count} terms | {terms_per_sec:.1f} terms/sec | {acceleration_status} | {processing_tier}")
                        last_performance_report = current_time
                
                except queue.Empty:
                    continue
                except Exception as e:
                    print(f"[ERROR] Hybrid worker {worker_id} error: {e}")
                    time.sleep(1.0)
                    continue
            ...
            print(f"[CONFIG] Hybrid CPU+GPU worker {worker_id} shutting down")
    ...
    def _pure_cpu_translation_worker(self, worker_id: int):
        ...
        try:
            # Use model loading lock to prevent race conditions
            with self.model_loading_lock:
                print(f"[CPU] CPU worker {worker_id}: Acquiring model loading lock...")
                ...
                model_name = f"facebook/nllb-200-{self.config.model_size}"
                print(f"[CPU] CPU worker {worker_id}: Loading CPU-only model: {model_name}")
                ...
                try:
                    ...
                    print(f"[OK] CPU worker {worker_id}: Pipeline created successfully (Method 1)")
                    
                except Exception as e1:
                    print(f"[WARNING]  CPU worker {worker_id}: Method 1 failed ({e1}), trying Method 2...")
                    ...
                    try:
                        ...
                        print(f"[OK] CPU worker {worker_id}: Pipeline created successfully (Method 2)")
                        
                    except Exception as e2:
                        print(f"[ERROR] CPU worker {worker_id}: Both methods failed. Method 1: {e1}, Method 2: {e2}")
                        print(f"[IDEA] CPU worker {worker_id}: Falling back to GPU queue delegation")
                        translator = None
                ...
                print(f"[OK] CPU worker {worker_id}: Model loaded successfully (CPU Mode)")
            
            print(f"[OK] Ultra CPU translation worker {worker_id} ready: {self.config.model_size} (CPU Mode)")
            ...
            if translator is None:
                print(f"[WARNING]  CPU worker {worker_id}: No translator available, delegating all work to GPU")
                while not self.stop_event.is_set():
                    try:
                        # Get work from CPU translation queue
                        item = self.cpu_translation_queue.get(timeout=2.0)
                        if item is None:  # Shutdown signal
                            print(f"[FAST] Ultra CPU translation worker {worker_id} received shutdown signal")
                            break
                        ...
                        try:
                            ...
                            processed_count += 1
                            print(f"[CONFIG] CPU worker {worker_id}: Delegated '{term}' to GPU queue {selected_gpu_idx + 1}")
                        except queue.Full:
                            print(f"[WARNING]  CPU worker {worker_id}: GPU queue full, skipping '{term}'")
                            
                    except queue.Empty:
                        continue
                    except Exception as e:
                        print(f"[ERROR] CPU worker {worker_id} delegation error: {e}")
                        continue
                        
                print(f"[OK] Ultra CPU translation worker {worker_id} completed (delegation mode): {processed_count} terms")
                return
                ...
                try:
                    ...
                    for lang in target_languages:
                        try:
                            ...
                            print(f"[WARNING]  CPU translation error for {term} -> {lang}: {e}")
                    ...
                    if processed_count % 5 == 0 or (current_time - last_performance_report) >= 10.0:
                        ...
                        print(f"[POWER] Ultra CPU-{worker_id}: {processed_count} terms | {terms_per_sec:.1f} terms/sec | {processing_tier} | avg: {avg_process_time:.1f}s")
                        last_performance_report = current_time
                        
                        # Stall detection
                        if terms_per_sec < 0.1 and processed_count > 0:  # Less than 0.1 terms/sec indicates stall
                            print(f"[WARNING]  CPU-{worker_id} PERFORMANCE WARNING: {terms_per_sec:.2f} terms/sec (possible stall)")
                    
                    # Regular progress reporting
                    elif processed_count % 5 == 0:
                        elapsed = time.time() - worker_start_time
                        rate = processed_count / elapsed if elapsed > 0 else 0
                        print(f"[POWER] Ultra CPU-{worker_id}: {processed_count} terms | {rate:.1f} terms/sec | CPU translation")
                
                except queue.Empty:
                    continue
                except Exception as e:
                    print(f"[WARNING]  CPU translation worker {worker_id} error: {e}")
            
            print(f"[OK] Ultra CPU translation worker {worker_id} completed: {processed_count} terms")
            
        except Exception as e:
            print(f"[ERROR] Ultra CPU translation worker {worker_id} initialization error: {e}")
    ...
    def _cpu_worker_ultra(self, worker_id: int, terms: List[str]):
        ...
        print(f"[FAST] Ultra CPU worker {worker_id} waiting for GPU workers...")
        ...
        if not all_gpus_ready:
            print(f"[WARNING]  Ultra CPU worker {worker_id}: Not all GPU workers ready, proceeding...")
        else:
            print(f"[OK] Ultra CPU worker {worker_id}: Ready for ultra-fast processing!")
            ...
            try:
                ...
                if processed_count % 10 == 0:
                    ...
                    if self.config.cpu_translation_workers > 0:
                        ...
                        print(f"[FAST] Ultra CPU-{worker_id}: {processed_count} terms | {selected_worker_type.upper()}-{selected_worker_id} | {processing_tier} | {efficiency:.1f}% eff | GPU:{gpu_load_pct:.0f}% CPU:{cpu_load_pct:.0f}%")
                    else:
                        print(f"[FAST] Ultra CPU-{worker_id}: {processed_count} terms | {selected_worker_type.upper()}-{selected_worker_id} | {processing_tier} | {efficiency:.1f}% efficiency")
                ...
                if selected_worker_type == 'gpu' and self.config.gpu_workers > 1:
                    ...
                    try:
                        ...
                        processed_count += 1
                        print(f"[FAST] Ultra CPU-{worker_id}: Switched to {selected_worker_type.upper()}-{alternate_worker_id}")
                    except queue.Full:
                        print(f"[WARNING]  Ultra CPU worker {worker_id}: All queues full, skipping term: {term}")
                else:
                    print(f"[WARNING]  Ultra CPU worker {worker_id}: Queue full, brief wait...")
                    time.sleep(0.2)
                    continue
            except Exception as e:
                print(f"[ERROR] Ultra CPU worker {worker_id} error: {e}")
                continue
        
        print(f"[OK] Ultra CPU worker {worker_id} completed: {processed_count} terms with ultra optimization")
    ...
    def _cpu_worker_ultra_continuous(self, worker_id: int):
        ...
        if not all_gpus_ready:
            ...
            print(f"[OK] Ultra CPU worker {worker_id}: Ready for continuous processing!")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if term is None:
                    print(f"[OK] Ultra CPU worker {worker_id} received stop signal")
                    break
                ...
                if self.work_queue.empty() and (self.processed_terms + self.failed_terms) >= self.total_terms:
                    print(f"[OK] Ultra CPU worker {worker_id}: All work completed")
                    break
                # Otherwise continue waiting for work
                continue
            except Exception as e:
                print(f"[ERROR] Ultra CPU worker {worker_id} fatal error: {e}")
                break
        
        print(f"[OK] Ultra CPU worker {worker_id} completed: {processed_count} terms with continuous processing")
    ...
    def _wait_for_all_terms_completion(self):
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if remaining <= 0:
                print(f"[OK] All {self.total_terms} terms completed successfully!")
                break
            ...
            if current_processed > last_processed:
                ...
                if stalled_count >= 12:  # 1 minute of no progress
                    print(f"[WARNING]  Progress stalled at {current_processed}/{self.total_terms} terms. Checking queues...")
                    ...
                    if gpu_queue_size == 0 and cpu_queue_size == 0:
                        print(f"[WARNING]  All queues empty but {remaining} terms not processed. Checking for stalled workers...")
                        ...
                        if stalled_count >= 120:  # 10 minutes of absolutely no progress
                            print(f"[ERROR] Translation appears permanently stalled. Processed {current_processed}/{self.total_terms} terms.")
        ...
        if final_processed < self.total_terms:
            print(f"[WARNING]  Translation incomplete: {final_processed}/{self.total_terms} terms processed")
        else:
            print(f"[OK] Translation complete: {final_processed}/{self.total_terms} terms processed")

    def _result_collector_ultra(self):
        """Ultra-optimized result collector with async capabilities"""
        print("[FAST] Starting ultra-optimized result collector...")
            ...
            try:
                ...
                if result.get('status') == 'completed':
                    ...
                    if self.processed_terms % 10 == 0:
                        ...
                        print(f"[FAST] Ultra: {self.processed_terms}/{self.total_terms} ({progress:.1f}%) | {rate:.3f}/sec | GPU-{gpu_worker} | {processing_tier} | Saved:{languages_saved}")
                else:
                    self.failed_terms += 1
                ...
                if self.processed_terms % 100 == 0 and self.processed_terms > 0:
                    ...
                    print(f"[FAST] ULTRA-OPTIMIZED PROGRESS:")
                ...
                print(f"[ERROR] Ultra result collector error: {e}")
                continue
        
        # Final save
        self._save_checkpoint_ultra()
        print("[FAST] Ultra-optimized result collector finished")
        ...
        try:
            ...
            if term and term not in existing_terms:
                ...
                if success:
                    ...
                    print(f"[OK] Added '{term}' to Translation_Results.json ({len(main_results_data['translation_results'])} total)")
                else:
                    print(f"[ERROR] Failed to save '{term}' - atomic write failed")
            
        except Exception as e:
            print(f"[ERROR] Failed to update main translation results: {e}")
    ...
    def _load_translation_results_safely(self, main_file, backup_file):
        ...
        if os.path.exists(main_file):
            try:
                ...
                print(f"[WARNING]  Main results file corrupted: {e}")
                
                # Try to recover from backup
                if os.path.exists(backup_file):
                    try:
                        print("[CONFIG] Attempting recovery from cleaned backup...")
                        with open(backup_file, 'r', encoding='utf-8') as f:
                            backup_data = json.load(f)
                        
                        # Restore main file from backup
                        self._atomic_json_write(main_file, backup_data)
                        print(f"[OK] Recovered {len(backup_data.get('translation_results', []))} entries from backup")
                        return backup_data
                        
                    except Exception as backup_error:
                        print(f"[ERROR] Backup recovery failed: {backup_error}")
                
                # Last resort: try to salvage partial data
                print("[SETUP] Attempting partial data recovery...")
    ...
    def _atomic_json_write(self, file_path, data):
        ...
        try:
            ...
            print(f"[ERROR] Atomic write failed for {file_path}: {e}")
    ...
    def _salvage_corrupted_json(self, file_path):
        ...
        try:
            ...
            if salvaged_results:
                print(f"[SETUP] Salvaged {len(salvaged_results)} translation results from corrupted file")
            ...
            print(f"[ERROR] Salvage operation failed: {e}")
        ...
        try:
            ...
            if main_checkpoint_file and os.path.exists(main_checkpoint_file):
                try:
                    ...
                    print(f"[OK] Main checkpoint updated: {self.processed_terms}/{self.total_terms} ({completion_percentage:.1f}%)")
                    
                except Exception as e:
                    print(f"[WARNING]  Failed to update main checkpoint: {e}")
            ...
            if self.processed_terms > 0:
                ...
                print(f"[SAVE] Progress saved: {self.processed_terms} terms | {efficiency:.1f}% efficiency | No separate files")
            
        except Exception as e:
            print(f"[WARNING]  Ultra checkpoint save error: {e}")
    ...
    def _save_unprocessed_term(self, term: str):
        ...
        try:
            ...
            if term not in unprocessed_terms:
                ...
                print(f"[SAVE] Saved unprocessed term for next session: {term}")
        except Exception as e:
            print(f"[WARNING] Could not save unprocessed term: {e}")
    ...
    def run_ultra_optimized_processing(self, terms_only=None):
        ...
        """
        print("[FAST] STARTING ULTRA-OPTIMIZED PROCESSING")
        print("=" * 60)
        
        try:
            if terms_only is not None:
                # Use only the provided terms (for translation step)
                print(f"[FAST] Processing specified terms: {len(terms_only)} terms")
                ...
                if self.total_terms == 0:
                    ...
                    print(f"[FAST] Fresh start: total_terms set to {self.total_terms}")
                else:
                    # Preserve total_terms from checkpoint (resume scenario)
                    print(f"[FAST] Resume mode: preserving total_terms={self.total_terms}, processing {len(all_terms)} remaining")
                ...
                if self.processed_terms == 0:
                    ...
                    self.failed_terms = 0
                    self.processed_terms_set.clear()
                    print(f"[FAST] Fresh start: reset counters for processing {len(all_terms)} terms")
                else:
                    # Preserve session progress when resuming
                    print(f"[FAST] Resume mode: preserving processed_terms={self.processed_terms}, continuing from session progress")
            else:
            ...
            print(f"[FAST] Processing {self.total_terms} terms with ULTRA-OPTIMIZED configuration:")
            
            # Show comprehensive resource allocation
            if self.config.gpu_workers > 0:
                print(f"   [GPU] GPU Translation: {self.config.gpu_workers}x {self.config.model_size} (batch={self.config.gpu_batch_size})")
                if self.config.cpu_translation_workers > 0:
                    print(f"   [POWER] CPU Translation: {self.config.cpu_translation_workers} workers (hybrid mode)")
                    print(f"   ⚙️  CPU Preprocessing: {self.config.cpu_workers} workers")
                    print(f"   [CONFIG] Intelligent Load Balancing: Simple->CPU, Complex->GPU")
            else:
                print(f"   [POWER] CPU-Only Translation: {self.config.cpu_translation_workers} workers")
                print(f"   ⚙️  CPU Preprocessing: {self.config.cpu_workers} workers")
            
            print(f"   [STATS] Total Workers: {self.config.gpu_workers + self.config.cpu_translation_workers + self.config.cpu_workers} + 3 system")
            print(f"   • Ultra-Minimal: {len(self.ultra_minimal_languages)} languages (90% efficiency)")
            print(f"   • Ultra-Core: {len(self.ultra_core_languages)} languages (80% efficiency)")
            print(f"   • Ultra-Extended: {len(self.ultra_extended_languages)} languages (60% efficiency)")
            ...
            if torch.cuda.is_available():
                ...
                torch.cuda.synchronize()
                gc.collect()
                print("[FAST] Ultra GPU memory optimization complete")
            ...
            if self.config.gpu_workers > 0:
                print(f"[GPU] Starting {self.config.gpu_workers} ultra-optimized GPU workers (sequential init with OOM fallback)...")
                ...
                for worker_id in range(1, self.config.gpu_workers + 1):
                    print(f"[GPU] Initializing GPU worker {worker_id}...")
                    ...
                    try:
                        ...
                        print(f"[GPU] Waiting for GPU worker {worker_id} to initialize...")
                        if not gpu_ready_event.wait(timeout=120):
                            print(f"[ERROR] GPU worker {worker_id} failed to initialize within 120 seconds")
                            failed_workers.append(worker_id)
                            # Don't raise exception, continue with fewer workers
                            continue
                        
                        print(f"[OK] GPU worker {worker_id} ready!")
                        successful_workers += 1
                        
                    except Exception as e:
                        print(f"[ERROR] GPU worker {worker_id} failed to start: {e}")
                        if "CUDA out of memory" in str(e) or "OutOfMemoryError" in str(e):
                            ...
                            print(f"[CONFIG] Continuing with {successful_workers} successfully initialized GPU workers")
                ...
                if successful_workers < self.config.gpu_workers:
                    print(f"[WARNING]  Only {successful_workers}/{self.config.gpu_workers} GPU workers started successfully")
                    if successful_workers == 0:
                        print(f"[ERROR] No GPU workers available - falling back to CPU-only mode")
                    else:
                        print(f"[OK] Continuing with {successful_workers} GPU workers (Tesla T4 OOM limitation)")
                ...
                print(f"[OK] All {len(gpu_threads)} GPU workers ready and operational")
            else:
                print("[FAST] CPU-only mode - no GPU workers started")
            ...
            if self.config.cpu_translation_workers > 0:
                print(f"[POWER] Starting {self.config.cpu_translation_workers} CPU translation workers (sequential init)...")
                
                # Add a small delay after GPU workers to avoid resource conflicts
                time.sleep(2)
                
                for i in range(self.config.cpu_translation_workers):
                    print(f"[POWER] Initializing CPU translation worker {i+1}...")
                ...
                print(f"[OK] All {len(cpu_translation_threads)} CPU translation workers started")
            
            # Start ultra CPU workers with continuous feeding approach
            print(f"[FAST] Starting {self.config.cpu_workers} ultra-optimized CPU workers...")
            print(f"[FAST] Using continuous work feeding to prevent queue starvation...")
            ...
            print(f"[FAST] Added {len(all_terms)} terms to work queue")
            ...
            with ThreadPoolExecutor(max_workers=self.config.cpu_workers, thread_name_prefix="UltraCPU") as executor:
                ...
                print(f"[OK] All ultra workers started!")
                print(f"[FAST] ULTRA-OPTIMIZED PROCESSING ACTIVE - CONTINUOUS FEEDING!")
                ...
                print("[OK] All terms processing confirmed - stopping work queue")
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        print(f"[ERROR] Ultra worker failed: {e}")
                
                print("[OK] All ultra CPU preprocessing workers completed")
                print("[OK] All terms processing confirmed - initiating shutdown sequence")
            ...
            for thread in gpu_threads:
                ...
                if thread.is_alive():
                    print(f"[WARNING]  Ultra GPU thread {thread.name} did not stop gracefully")
            
            # Wait for CPU translation workers
            for thread in cpu_translation_threads:
                thread.join(timeout=30)
                if thread.is_alive():
                    print(f"[WARNING]  CPU translation thread {thread.name} did not stop gracefully")
            ...
            print("\n[FAST] ULTRA-OPTIMIZED PROCESSING COMPLETED!")
            print("=" * 60)
            print(f"[STATS] ULTRA PERFORMANCE RESULTS:")
            ...
            print(f"   [FAST] ULTRA OPTIMIZATION BREAKDOWN:")
            ...
            print(f"   • GPU Performance: GPU1:{self.gpu_performance[0]:.2f} GPU2:{self.gpu_performance[1]:.2f} items/sec")
            print(f"   [FAST] MAXIMUM PERFORMANCE ACHIEVED!")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--resume-from" and len(sys.argv) > 2:
        ...
        print(f"[FAST] Starting ultra-optimized runner from session: {session_id}")
        config = UltraOptimizedConfig()
        runner = UltraOptimizedSmartRunner(config=config, resume_session=session_id)
        runner.run_ultra_optimized_processing()
    else:
        # Start fresh
        print("[FAST] Starting fresh ultra-optimized processing...")
        config = UltraOptimizedConfig()
