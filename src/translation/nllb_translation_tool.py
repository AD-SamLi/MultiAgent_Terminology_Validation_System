...
class NLLBTranslationTool:
    ...
    def __init__(self, model_name: str = "facebook/nllb-200-1.3B", device: str = "auto", batch_size: int = 8, offload_dir: str = None):
        ...
        print(f"[LAUNCH] Initializing NLLB-200 Translation Tool")
        ...
        print(f"[OK] NLLB-200 tool ready with {len(self.NLLB_LANGUAGES)} languages")
    ...
    def _setup_device(self, device: str) -> str:
        ...
        if device == "auto":
            if torch.cuda.is_available():
                device = "cuda"
                print(f"[GPU] CUDA available: {torch.cuda.get_device_name()}")
            ...
            print(f"[GPU] Specific CUDA device requested: {device}")
    ...
    def _load_model(self):
        ...
        if self.device == "cuda" and torch.cuda.is_available():
            print("[GPU] Using GPU + CPU hybrid loading with optimized memory management...")
            ...
            if torch.cuda.device_count() > 0:
                ...
                if "Tesla T4" in gpu_name:
                    ...
                    print(f"   [TARGET] Tesla T4 detected: Using {max_memory_per_device} max memory (95% of {gpu_memory_gb:.1f}GB)")
                else:
                    # Other GPUs: Use 92% of memory
                    max_memory_per_device = f"{gpu_memory_gb * 0.92:.1f}GB"
                    print(f"   [TARGET] {gpu_name}: Using {max_memory_per_device} max memory (92% of {gpu_memory_gb:.1f}GB)")
            ...
            print("[CPU] Using CPU with memory optimization...")
            load_kwargs = {
        ...
        print("[OK] Model loaded with hybrid memory management")
        
        # Print memory info if CUDA is available
        if torch.cuda.is_available():
            allocated = torch.cuda.memory_allocated() / 1e9
            reserved = torch.cuda.memory_reserved() / 1e9
            print(f"[GPU] GPU Memory - Allocated: {allocated:.1f}GB, Reserved: {reserved:.1f}GB")
        
        # Create translation pipeline (let accelerate handle device placement)
        print("[SETUP] Creating translation pipeline...")
        ...
        print("[OK] Translation pipeline ready")
    ...
    def translate_to_all_languages(self, text: str, src_lang: str = "eng_Latn") -> Dict[str, TranslationResult]:
        ...
        for i in range(0, len(target_languages), batch_size):
            ...
            print(f"[PACKAGE] Processing batch {i//batch_size + 1}/{(len(target_languages) + batch_size - 1)//batch_size}")
            ...
            for tgt_lang in batch_langs:
                try:
                    ...
                    if result.error:
                        print(f"[WARNING]  {tgt_lang}: {result.error}")
                    elif result.is_same:
                        print(f"🟡 {tgt_lang}: Same as original")
                    else:
                        print(f"[OK] {tgt_lang}: Translated")
                    ...
                    print(f"[ERROR] {tgt_lang}: Failed - {str(e)}")
