...
[OK] **Successfully converted PRDSMRT data to system-compatible format**
...
Examples:
  ...
  python convert_prdsmrt_to_system_format.py PRDSMRT_doc_merged_results_Processed_Simple.csv --output-dir ./converted --min-frequency 5
    ...
    try:
        ...
        print(f"\n[OK] SUCCESS: Conversion completed!")
        print(f"[STATS] Stats: {result['stats']['high_frequency_terms']:,} high-frequency terms ready for validation")
        print(f"[FOLDER] Files: {result['high_frequency_file']}")
        print(f"[BOOK] Instructions: {result['instructions_file']}")
