...
def robust_agent_call_with_retries(agent_func: Callable, *args, max_retries: int = 3, base_delay: float = 2.0, **kwargs) -> Any:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if hasattr(agent_func, '__self__'):
                ...
                if hasattr(agent_instance, 'model') and hasattr(agent_instance.model, '_refresh_token_if_needed'):
                    if agent_instance.model._refresh_token_if_needed():
                        print(f"[CONFIG] [AuthFix] Proactive token refresh triggered for {agent_instance.__class__.__name__}")
            ...
            if attempt > 0:
                print(f"[OK] [AuthFix] Function succeeded on attempt {attempt + 1}")
            
            return result
            
        except Exception as e:
            error_msg = str(e)
            print(f"[WARNING] [AuthFix] Attempt {attempt + 1}/{max_retries} failed: {error_msg[:200]}...")
            
            if attempt == max_retries - 1:
                print(f"[ERROR] [AuthFix] All {max_retries} attempts failed")
                raise e
            
            # Determine retry strategy based on error type
            if is_token_expired(error_msg):
                print("[CONFIG] [AuthFix] Token expired - forcing refresh...")
                ...
                if agent_instance and hasattr(agent_instance, 'refresh_model_token'):
                    try:
                        agent_instance.refresh_model_token()
                        print("[OK] [AuthFix] Token refresh completed")
                    except Exception as refresh_error:
                        print(f"[ERROR] [AuthFix] Token refresh failed: {refresh_error}")
                        
            elif is_server_error(error_msg):
                delay = base_delay * (2 ** attempt)  # Exponential backoff for server errors
                print(f"[CONFIG] [AuthFix] Server error - exponential backoff ({delay:.1f}s)")
                
            elif is_content_filter_error(error_msg):
                delay = base_delay * (2 ** attempt)  # Exponential backoff for content filter
                print(f"[CONFIG] [AuthFix] Content filter error - exponential backoff ({delay:.1f}s)")
                
            else:
                delay = base_delay * (1.5 ** attempt)  # Standard backoff
                print(f"[CONFIG] [AuthFix] General error - standard backoff ({delay:.1f}s)")
...
def apply_auth_fix_to_agent(agent_instance):
    ...
    for method_name in api_methods:
        if hasattr(agent_instance, method_name):
            ...
            print(f"[OK] [AuthFix] Applied authentication fix to {agent_instance.__class__.__name__}.{method_name}")
...
def ensure_agent_auth_fix(agent_instance):
    ...
    if not hasattr(agent_instance, '_auth_fix_applied'):
        print(f"[SETUP] [AuthFix] Applying authentication fixes to {agent_instance.__class__.__name__}")
        return apply_auth_fix_to_agent(agent_instance)
    else:
        print(f"[OK] [AuthFix] Authentication fixes already applied to {agent_instance.__class__.__name__}")
        return agent_instance
...
def test_agent_auth_fix():
    ...
    try:
        ...
        print("[OK] TerminologyAgent test passed")
        ...
        print("[OK] ModernTerminologyReviewAgent test passed")
        
        print("\n[SUCCESS] All authentication fix tests passed!")
        return True
        
    except Exception as e:
        print(f"[ERROR] Authentication fix test failed: {e}")
        return False
