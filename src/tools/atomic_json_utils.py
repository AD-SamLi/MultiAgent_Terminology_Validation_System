...
def load_json_safely(file_path, backup_paths=None, create_empty_on_fail=True):
    ...
    if os.path.exists(file_path):
        try:
            ...
            print(f"[WARNING]  Primary file corrupted ({file_path}): {e}")
            ...
            if backup_paths:
                for backup_path in backup_paths:
                    if os.path.exists(backup_path):
                        try:
                            print(f"[CONFIG] Attempting recovery from {backup_path}...")
                            with open(backup_path, 'r', encoding='utf-8') as f:
                                backup_data = json.load(f)
                            
                            # Restore primary file from backup
                            if atomic_json_write(file_path, backup_data):
                                print(f"[OK] Recovered from {backup_path}")
                                return backup_data
                            
                        except Exception as backup_error:
                            print(f"[ERROR] Backup {backup_path} failed: {backup_error}")
                            continue
            
            # Last resort: try to salvage partial data
            print("[SETUP] Attempting data salvage...")
...
def salvage_json_data(file_path):
    ...
    try:
        ...
        if len(salvaged_data) > 1:  # More than just metadata
            print(f"[SETUP] Salvaged data from {file_path}:")
...
def test_corruption_resistance():
    ...
    try:
        ...
        print(f"[OK] Atomic write: {'SUCCESS' if success else 'FAILED'}")
        
        # Test 2: Safe load
        loaded_data = load_json_safely(test_file)
        print(f"[OK] Safe load: {'SUCCESS' if loaded_data and loaded_data.get('test') else 'FAILED'}")
        ...
        print(f"[OK] Corruption recovery: {'SUCCESS' if recovered_data and recovered_data.get('test') else 'FAILED'}")
