...
class MultiModelGPUManager:
    ...
    def create_optimal_allocation_plan(self, required_models: List[str]) -> MultiModelGPUConfig:
        ...
        if total_gpus >= 2:
            ...
            for model in sorted_models:
                ...
                if best_gpu is not None:
                    ...
                    logger.warning(f"[WARNING] Fallback: Allocated '{model}' to GPU {fallback_gpu} (may exceed memory)")
        ...
        logger.info("[TARGET] Final GPU Allocation Plan:")
    ...
    def get_optimized_worker_config(self, allocation_plan: MultiModelGPUConfig) -> Dict[str, Any]:
        ...
        logger.info(f"   [GPU] GPU Workers: {gpu_workers} (across {active_gpus} active GPUs)")
        logger.info(f"   [POWER] CPU Workers: {cpu_workers}")
        logger.info(f"   [CONFIG] Preprocessing: {preprocessing_workers}")
        logger.info(f"   [STATS] Models: {total_models} total across {active_gpus} GPUs")
...
if __name__ == "__main__":
    ...
    if config['success']:
        print("[OK] Multi-Model GPU Configuration Generated Successfully")
        ...
        print(f"[ERROR] Failed to generate configuration: {config['error']}")
