...
class MultiModelGPUManager:
    ...
    def _detect_gpu_capabilities(self) -> Dict[int, Dict[str, Any]]:
        ...
        logger.info(f"[SEARCH] Detected {gpu_count} CUDA-capable GPU(s)")
    ...
    def add_custom_model(self, model_spec: ModelSpec):
        ...
        logger.info(f"[LOG] Added custom model: {model_spec.name} ({model_spec.memory_requirement_gb:.1f}GB)")
        ...
        for model in models:
            ...
            for allocation in sorted(allocations, key=lambda x: x.available_memory_gb, reverse=True):
                if self._can_fit_model(model, allocation):
                    ...
                    if self._is_compatible_placement(model, allocation):
                        ...
                        allocated = True
                        logger.info(f"[TARGET] Allocated {model.name} to GPU {allocation.gpu_id} (multi-model)")
                        break
            ...
            if not allocated:
                ...
                if self._can_fit_model(model, best_gpu):
                    ...
                    allocated = True
                    logger.info(f"[TARGET] Allocated {model.name} to GPU {best_gpu.gpu_id} (best fit)")
            
            if not allocated:
                logger.warning(f"[WARNING] Could not allocate {model.name} - insufficient GPU memory")
    ...
    def _log_allocation_plan(self, allocations: List[GPUAllocation]):
        ...
        logger.info("[TARGET] MULTI-MODEL GPU ALLOCATION PLAN:")
        ...
        logger.info(f"   [STATS] Total Models: {total_models}")
        logger.info(f"   [GPU] Active GPUs: {active_gpus}/{len(allocations)}")
    ...
    def export_allocation_config(self, filepath: str = None) -> str:
        ...
        logger.info(f"[FILE] Multi-model allocation config saved to: {filepath}")
        return filepath
...
if __name__ == "__main__":
    ...
    print("[SEARCH] Testing Multi-Model GPU Allocation...")
    ...
    try:
        ...
        print(f"[OK] Successfully created allocation for {len(test_models)} models")
        
        # Show allocation summary
        for allocation in allocations:
            if allocation.allocated_models:
                print(f"[GPU] GPU {allocation.gpu_id}: {len(allocation.allocated_models)} models, {allocation.memory_utilization_percent:.0f}% utilization")
        ...
        print(f"[LAUNCH] Optimized batch sizes: {len(optimized_batches)} models")
