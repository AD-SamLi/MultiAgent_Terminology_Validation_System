...
class AdaptiveSystemDetector:
                    ...
                    if torch.cuda.is_available():
                        for gpu_id in range(torch.cuda.device_count()):
                            ...
                            if available > 4.0:  # Need at least 4GB for multi-model
                                ...
                                logger.info(f"🎮 GPU {gpu_id} supports multi-model: {available:.1f}GB available")
                                break
                except Exception as e:
    ...
    def _generate_adaptive_config(self) -> AdaptiveConfig:
        ...
        logger.info(f"🎯 Generated {profile.estimated_performance_tier.upper()} performance configuration")
    ...
    def _get_extreme_performance_config(self, profile: SystemProfile) -> AdaptiveConfig:
        ...
        if profile.gpu_count > 1:
            ...
            if profile.supports_multi_model_gpu:
                ...
                logger.info(f"🎮 OPTIMIZED Single GPU Multi-Model: {max_nllb_instances} NLLB instances → {gpu_workers} GPU workers")
        ...
        logger.info(f"🔄 EXTREME TRANSLATION-OPTIMIZED: {gpu_workers} GPU ({max_nllb_instances} NLLB) + {cpu_translation_workers} CPU-Trans + {cpu_workers} CPU-Main = {total_workers}/{available_cores} cores")
        ...
        if enable_multi_model:
            ...
            try:
                ...
                logger.info(f"🎮 Multi-model GPU allocation: {len(models_per_gpu)} active GPUs")
            except Exception as e:
    ...
    def _get_high_performance_config(self, profile: SystemProfile) -> AdaptiveConfig:
        ...
        if profile.gpu_count > 1:
            ...
            if profile.supports_multi_model_gpu:
                ...
                gpu_workers = min(max_nllb_instances, 3)  # Increase cap from 2 to 3
                logger.info(f"🎮 OPTIMIZED High-Perf GPU Multi-Model: {max_nllb_instances} NLLB instances → {gpu_workers} GPU workers")
    ...
    def _apply_memory_constraints(self, config: AdaptiveConfig, profile: SystemProfile) -> AdaptiveConfig:
        ...
        if memory_per_worker < 0.5:  # Less than 512MB per worker (very aggressive)
            ...
            logger.info(f"⚠️ Aggressive memory allocation: {memory_per_worker:.2f}GB per worker")
        ...
        if available_memory_per_worker > 2.0:  # More than 2GB per worker
            ...
            logger.info(f"🚀 Memory scaling applied: {memory_multiplier:.1f}x batch size increase")
    ...
    def _apply_gpu_optimizations(self, config: AdaptiveConfig, profile: SystemProfile) -> AdaptiveConfig:
        ...
        if profile.gpu_count == 0:
            ...
            logger.info("🔧 No GPU detected - boosting CPU workers for compensation")
        else:
            ...
            if total_gpu_memory >= 16:
                ...
                logger.info(f"🚀 High GPU memory ({total_gpu_memory:.1f}GB) - maximum batch sizes enabled")
                ...
                config.enable_model_parallel = profile.gpu_count >= 2
                logger.info(f"⚡ Medium GPU memory ({total_gpu_memory:.1f}GB) - optimized configuration")
                ...
                logger.info(f"🔧 Low GPU memory ({total_gpu_memory:.1f}GB) - efficient configuration")
                ...
                logger.info(f"⚠️ Very low GPU memory ({total_gpu_memory:.1f}GB) - quantized models")
            ...
            if profile.gpu_count >= 2:
                ...
                logger.info(f"🎮 Multi-GPU setup ({profile.gpu_count} GPUs) - parallel processing enabled")
            ...
            if memory_per_gpu > 6:  # More than 6GB per GPU
                ...
                logger.info(f"📈 GPU memory scaling: {scaling_factor:.1f}x for {memory_per_gpu:.1f}GB per GPU")
        
        return config
    
    def _log_system_profile(self, profile: SystemProfile):
        """Log detected system profile"""
        logger.info("🖥️  SYSTEM PROFILE DETECTED:")
    ...
    def _log_adaptive_config(self, config: AdaptiveConfig):
        ...
        if config.enable_multi_model_gpu:
            ...
            logger.info(f"   🎮 Multi-Model GPU: {total_models} models across {active_gpus} GPUs")
...
class DynamicResourceMonitor:
    ...
    def start_monitoring(self, adjustment_interval: float = 30.0):
        ...
        logger.info(f"🔄 Dynamic resource monitoring started (interval: {adjustment_interval}s)")
    ...
    def _check_and_adjust_resources(self):
        ...
        if adjustments_made:
            ...
            logger.info(f"🔧 [{timestamp}] Dynamic adjustments: {', '.join(adjustments_made)}")
...
def get_adaptive_system_config(enable_dynamic_monitoring: bool = True) -> Tuple[SystemProfile, AdaptiveConfig, Optional[DynamicResourceMonitor]]:
    ...
    if enable_dynamic_monitoring:
        ...
        logger.info("🎯 Dynamic resource monitoring enabled for maximum utilization")
    
    return profile, config, monitor

if __name__ == "__main__":
    # Test adaptive configuration
    print("🔍 Testing Adaptive System Configuration...")
    
    try:
        profile, config, monitor = get_adaptive_system_config()
        print(f"✅ Successfully detected {profile.estimated_performance_tier.upper()} performance system")
        print(f"📊 Configuration: {config.gpu_workers} GPU + {config.cpu_workers} CPU workers")
        ...
        if monitor:
            print("🔄 Dynamic resource monitoring: ENABLED")
