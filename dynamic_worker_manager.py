...
class DynamicWorkerManager:
    ...
    def update_queue_status(self, queue_name: str, current_queue: queue.Queue):
        ...
        try:
            ...
            print(f"⚠️  Failed to update queue status for {queue_name}: {e}")
    ...
    def should_throttle_queue(self, queue_name: str) -> bool:
        ...
        if status.fullness >= 0.60:  # Lowered from 0.80 for better flow
            ...
            if self.worker_allocation.idle_workers > 0:
                print(f"🔄 Queue {queue_name} busy ({status.fullness:.1%}) - redeploying {self.worker_allocation.idle_workers} idle workers")
    ...
    def _redeploy_idle_workers_for_translation(self):
        ...
        with self.lock:
            ...
            if resources['cpu_percent'] > 85 or resources['memory_percent'] > 90:
                print(f"⚠️  System overloaded (CPU: {resources['cpu_percent']:.1f}%, RAM: {resources['memory_percent']:.1f}%) - skipping worker redeployment")
                return
            ...
            print(f"🔄 DYNAMIC REDEPLOYMENT: Moving {workers_to_redeploy} idle workers to translation")
    ...
    def intelligent_queue_throttling(self, queue_name: str, current_queue: queue.Queue, 
        ...
        if not self.should_throttle_queue(queue_name):
            ...
            try:
                ...
                print(f"⚠️  Queue {queue_name} full despite no throttling - system overload")
        ...
        while time.time() - throttle_start < max_wait:
            ...
            if current_status.fullness <= self.config.queue_resume_threshold:
                ...
                print(f"✅ THROTTLING RESUMED: {queue_name} cleared to {current_status.fullness:.1%} after {elapsed:.1f}s")
                ...
                try:
                    ...
                    print(f"⚠️  Queue {queue_name} filled again immediately - continuing throttling")
                    continue
            
            # Show progress every 5 seconds
            elapsed = time.time() - throttle_start
            if int(elapsed) % 5 == 0 and int(elapsed) > 0:
                print(f"   🔄 Still throttled: {queue_name} {current_status.fullness:.1%} full, waiting {elapsed:.0f}s...")
            
            time.sleep(self.config.throttle_check_interval)
        
        # Force resume after max wait
        print(f"⚠️  FORCE RESUME: {queue_name} throttling timeout ({max_wait}s) - forcing queue attempt")
        return self._handle_queue_full_fallback(queue_name, current_queue, work_item, timeout)
    ...
    def _handle_queue_full_fallback(self, queue_name: str, current_queue: queue.Queue, 
        ...
        try:
            ...
            print(f"❌ QUEUE OVERFLOW: {queue_name} cannot accept items - implementing fallback")
            ...
            if (resources['cpu_percent'] < 70 and resources['memory_percent'] < 80 and 
                ...
                print(f"🚀 EMERGENCY SCALING: Spawning additional translation worker for {queue_name}")
            ...
            print(f"💾 PERSISTENT QUEUE: Saving work item for later processing")
    ...
    def _spawn_emergency_translation_worker(self, queue_name: str):
        ...
        with self.lock:
            ...
            print(f"⚡ Emergency worker spawned for {queue_name}")
    ...
    def _emergency_queue_handling(self, queue_name: str):
        ...
        if self.worker_allocation.idle_workers > 0:
            print(f"🚀 Emergency redeployment: {self.worker_allocation.idle_workers} workers")
        ...
        print(f"🔄 Emergency measure: Requesting worker performance check for {queue_name}")
    ...
    def _force_emergency_drain(self, queue_name: str):
        ...
        print(f"🚀 Requesting emergency worker boost for {queue_name}")
    ...
    def _save_to_persistent_queue(self, queue_name: str, work_item):
        ...
        print(f"💾 Saved work item to persistent queue for {queue_name}")
...
def integrate_with_ultra_runner(runner_instance, worker_manager):
    ...
    print("✅ Dynamic worker manager integrated with UltraOptimizedSmartRunner")
